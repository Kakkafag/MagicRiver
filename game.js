...
function update(dt) {
  ...
  if (state.mode === 'menu') {
    ...
    state.spawnTimer -= dt;
    const max = Math.min(8, 3 + Math.floor(state.score / 650));
    if (!state.started) { for (let i = 0; i < 2; i++) spawnAnimal(); state.started = true; state.spawnTimer = 1.10; }
    if (state.spawnTimer <= 0 && state.animals.length < max) { spawnAnimal(); state.spawnTimer = Math.max(.48, Math.random() * .55 + .68 - Math.max(0, state.score - 250) / 3200); }
...
function spawnAnimal() {
  ...
  const level = Math.min(12, Math.floor(state.score / 180));
  const speed = t.speed * (type === 'GOLDEN_PLATYPUS' ? .92 : .68) * (1 + Math.min(.65, level*.045)) + Math.random()*22 + Math.max(0, state.score-250)*.018;
