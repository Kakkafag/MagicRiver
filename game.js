...
function update(dt) {
  ...
  if (state.mode === 'menu') {
    ...
    const max = Math.min(12, 5 + Math.floor(difficulty * 7));
    if (!state.started) { for (let i = 0; i < 4; i++) spawnAnimal(); state.started = true; state.spawnTimer = 0.72; }
    if (state.spawnTimer <= 0 && state.animals.length < max) {
      spawnAnimal();
      const baseSpawn = 0.70 - difficulty * 0.38;
      state.spawnTimer = Math.max(0.25, baseSpawn + Math.random() * 0.28);
    }
...
function spawnAnimal() {
  ...
  const scoreBonus =
    Math.max(0, state.score - 1200) * 0.006;

  const speed =
    t.speed * typeMultiplier * speedMultiplier +
    Math.random() * (20 + difficulty * 24) +
    scoreBonus;
